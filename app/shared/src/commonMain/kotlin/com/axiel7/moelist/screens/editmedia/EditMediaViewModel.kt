...
class EditMediaViewModel(
    ...
    override fun setEditVariables(myListStatus: BaseMyListStatus) {
        mutableUiState.update {
            it.copy(
                status = myListStatus.status.toBo(),
    ...
    override fun onChangeStatus(value: ListStatus) {
        mutableUiState.update {
            it.copy(
                status = value,
                startDate = if (it.isPlanned && value.isCurrent()) today else it.startDate,
                finishDate = if (value == ListStatus.COMPLETED) today else it.finishDate,
    ...
    override fun onChangeProgress(value: Int?) {
        mutableUiState.update {
            ...
            if (canChangeProgressTo(value, progressLimit)) {
                ...
                val startDate = if (isUpdatingFromPlanning) today else it.startDate
                val finishDate = if (isUpdatingToLastProgress) today else it.finishDate
    ...
    override fun updateListItem() {
        viewModelScope.launch(Dispatchers.IO) {
            mutableUiState.value.run {
                ...
                val statusValue = if (status != myListStatus?.status?.toBo())
                    status else null
