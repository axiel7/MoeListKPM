...
class DefaultPreferencesRepository(
    ...
    suspend fun nsfwInt() = nsfw.first().let { if (it) 1 else 0 }
    ...
    suspend fun setPaletteStyle(value: PaletteStyle) {
        dataStore.setValue(PALETTE_STYLE_KEY, value.name)
    }
    ...
    val animeListStatus = dataStore.getValue(ANIME_LIST_STATUS_KEY, ListStatusDto.WATCHING.name)
        .map { ListStatusDto.valueOf(it) }

    suspend fun setAnimeListStatus(value: ListStatusDto) {
        dataStore.setValue(ANIME_LIST_STATUS_KEY, value.name)
    }

    val mangaListStatus = dataStore.getValue(MANGA_LIST_STATUS_KEY, ListStatusDto.READING.name)
        .map { ListStatusDto.valueOf(it) }

    suspend fun setMangaListStatus(value: ListStatusDto) {
